...
async def main():
    ...
    owner_phone_no = "7010942886"
    if not (owner_phone_no.startswith("+91")) and len(owner_phone_no) == 10:
        owner_phone_no = "+91" + owner_phone_no
    elif not (owner_phone_no.startswith("+")) and len(owner_phone_no) == 12:
        owner_phone_no = "+" + owner_phone_no
    flag = 0
    for each_user in get_contact_result.users:
        if each_user.phone == owner_phone_no[1:]:
            flag = 1
            break
    if flag == 0:
        contact = InputPhoneContact(client_id=0, phone=owner_phone_no,
                                    first_name=owner_name + company_string, last_name="")
        result = await client(ImportContactsRequest([contact]))
    entity = await client.get_entity(owner_phone_no)
    for customer_id in customer_id_phone_no_mappings:
        bill_exists = False
